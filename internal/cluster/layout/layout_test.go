...
func TestLayoutShardIDs(t *testing.T) {
	layout, err := NewModuloLayout([]int{1, 2}, 4)
	if err != nil {
		t.Fatalf("NewModuloLayout returned error: %v", err)
	}

	shardIDs := layout.ShardIDs()

	if len(shardIDs) != 4 || shardIDs[0] != 0 || shardIDs[1] != 1 || shardIDs[2] != 2 || shardIDs[3] != 3 {
		t.Fatalf("shards mismatch: got %v, want [0 1 2 3]", shardIDs)
	}
}
