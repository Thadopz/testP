...
10. 为 Order Worker 增加按 shard 微批事务：连续事件仍按顺序应用，但一批只开启一次事务并只写一次批末 checkpoint。

微批没有改变一致性边界。某批中任一事件失败时，Inbox、订单状态、Outbox 和 checkpoint 会一起回滚；重启后仍从旧 checkpoint 重放。当前批量化减少的是事务开启、提交和 checkpoint 更新次数，批内业务 SQL 仍逐条执行，Matcher Worker也仍逐条处理匹配请求。

## 4. 过往的废案

- 由于是预先规划接口，先做了内存和文件态的实现，验证了事件驱动和 checkpoint 恢复的可行性，但没有考虑分布式场景下的 ownership、fencing 和跨资源一致性问题，此外，遗留的旧功能test与benchmark也非常多，还有些功能懒加载混杂在主链路里。
- 当初没有考虑到engine的功能必须拆分，在分发事件之后就直接调用单机模型的engine，导致在分布式场景下，engine的状态和数据库事务混在一起，难以保证一致性。
- 由于我是边扩充新技术栈边落实的，对于一些新技术栈的使用和设计没有提前规划好，导致在实现过程中出现了很多不必要的复杂性和重复工作，比如让etcd承担了过多的职责，早期的checkpoint是由etcd负责的。
- 对于跨shard查找曾经也按照单机模型内的算法实现过，但是性能实在太差，目前还在考虑如何拆分成独立的RPC服务，或者在数据库侧做条件查询（会不会更慢了？）。
- 负载均衡的实现也有过多次尝试，最初是通过etcd的watch来感知节点变化，然后重新计算shard的归属，但是这种方式在节点频繁变动时会导致大量的ownership变更，影响系统稳定性。后来改为使用一致性哈希算法来分配shard，但是在节点数量较少时仍然存在分布不均的问题。
- 曾经尝试直接使用kafka consumer group来管理membership和ownership，但是由于当时还是使用单机engine，对于分布式场景下的状态管理存在困难，当正式将engine替换后又感觉没必要换掉etcd实现的membership和ownership了，因为性能瓶颈还是在SQL上。
- 对于控制器选举leader，最开始是手写了raft算法来实现，但是由于etcd已经提供了成熟的leader election机制，所以最终还是选择使用etcd来管理controller的leader选举。