...
项目从先前的单机订单匹配 benchmark 出发，在分布式方向上进行了拓展：事件日志、分片拥有权、持久化、节点故障切换和可观测指标等功能，初步确保了一致性与幂等。
...
注：在后来的设计中没有考虑到matching engine仍然是内存里处理，内部状态无法聚合
